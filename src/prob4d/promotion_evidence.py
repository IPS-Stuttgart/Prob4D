        ...
        result.append(
            {
                ...
                "provider_method_id": (
                    None
                    if provider is None
                    else _text(provider, "provider_method_id")
                ),
                "query_method_id": _text(
                    arm.get("query_method_id"),
                    "query_method_id",
                ),
                "sensor_assisted": _boolean(
                    arm.get("sensor_assisted"),
                    "sensor_assisted",
                ),
            }
        )
    if [row["arm_id"] for row in result] != sorted(
...
def build_promotion_evidence_card(
    ...
    if (report.get("schema_name"), report.get("schema_version")) != (
        _REPORT_SCHEMA,
        1,
    ):
    ...
    development = _ids(
        lock.get("development_group_ids"),
        "development_group_ids",
    )
    calibration = _ids(
        lock.get("calibration_group_ids"),
    ...
    descriptor: dict[str, Any] = {
        ...
        "repositories": {
            "prob4d": {
                ...
                "revision": _revision(
                    lock.get("source_revision"),
                    "source_revision",
                ),
            },
            "bayesian_phystwin": {
                "repository": _text(
                    lock.get("bayesian_phystwin_repository"),
                    "bayesian_phystwin_repository",
                ),
                "revision": _revision(
                    lock.get("bayesian_phystwin_revision"),
                    "bayesian_phystwin_revision",
                ),
            },
            "motioncrafter": {
                "revision": _revision(
                    lock.get("motioncrafter_revision"),
                    "motioncrafter_revision",
                ),
                "model_set_id": _digest(
                    lock.get("model_set_id"),
                    "model_set_id",
                ),
            },
        },
        ...
        "frozen_inputs": {
            "prediction_run_spec_id": _digest(
                lock.get("prediction_run_spec_id"),
            ...
            "frozen_artifact_ids": dict(
                _map(lock.get("frozen_artifact_ids"), "artifacts")
            ),
            "provider_report_sha256": _digest(
                report.get("provider_report_sha256"),
                "provider_report_sha256",
            ),
            "query_results_id": _digest(
                report.get("query_results_id"),
                "query_results_id",
            ),
        },
        "provider_gate": {
            "passed": _boolean(
                provider.get("overall_passed"),
                "provider passed",
            ),
            "reference_method": _text(
                audit.get("reference_method"),
                "reference_method",
            ),
        ...
        "guarded_query": {
            ...
            "physical_fallback_arm_id": _text(
                query.get("physical_fallback_arm_id"),
                "physical_fallback_arm_id",
            ),
            "paired_candidate_minus_fallback_mm": {
                "mean": _real(paired.get("mean"), "paired mean"),
                "ci95_lower": _real(
                    paired.get("ci95_lower"),
                    "paired lower",
                ),
                "ci95_upper": _real(
                    paired.get("ci95_upper"),
                    "paired upper",
                ),
                "group_count": _integer(
                    paired.get("group_count"),
                    "paired groups",
                ),
                "semantics": _text(
                    paired.get("semantics"),
                    "paired semantics",
                ),
            },
            "mean_query_rmse_mm": _real(
                primary.get("mean_query_rmse_mm"),
                "mean RMSE",
            ),
            "accepted_update_count": _integer(
                primary.get("accepted_update_count"),
                "accepted updates",
            ),
            "rejected_update_count": _integer(
                primary.get("rejected_update_count"),
                "rejected updates",
            ),
            "harmful_accepted_update_count": _integer(
                primary.get("harmful_accepted_update_count"),
                "harmful updates",
            ),
            "technical_failure_count": _integer(
                primary.get("technical_failure_count"),
                "technical failures",
            ),
            "worst_group_regression_mm": _real(
                primary.get("worst_group_regression_mm"),
                "worst regression",
            ),
            "mean_accepted_coverage": (
                None
                if primary.get("mean_accepted_coverage") is None
                else _real(
                    primary.get("mean_accepted_coverage"),
                    "accepted coverage",
                )
            ),
            "mean_accepted_width_mm": (
                None
                if primary.get("mean_accepted_width_mm") is None
                else _real(
                    primary.get("mean_accepted_width_mm"),
                    "accepted width",
                )
            ),
            "exact_fallback_failure_count": _integer(
                query.get("exact_fallback_failure_count"),
                "fallback failures",
            ),
        },
...
def promotion_evidence_card_from_dict(value: object) -> dict[str, Any]:
    ...
    descriptor = {
        key: item for key, item in card.items() if key != "evidence_card_id"
    }
