...
class SelectedOverlapWindow:
    """One independently decoded window admitted before opening future payloads."""
    ...
    def __post_init__(self) -> None:
        ...
        _require_sha256(self.payload_sha256, name="overlap-window payload_sha256")
...
class CausalOverlapSelection:
    """Causally complete overlap windows and their append-invariant source digest."""
    ...
    def __post_init__(self) -> None:
        if not self.windows:
            raise ValueError("causal overlap selection must contain at least one window")
        if self.skipped_window_count < 0:
            raise ValueError("skipped_window_count must be nonnegative")
        _require_sha256(self.manifest_sha256, name="prediction manifest sha256")
        _require_sha256(self.source_artifact_sha256, name="causal source sha256")

    @property
    def predictions(self) -> list[PredictionWindow]:
        return [item.prediction for item in self.windows]

    def artifact_lineage_metadata(self, *, causal_frame_stop: int) -> dict[str, Any]:
        ...
        return {
            ...
            "selected_windows": [item.lineage_record() for item in self.windows],
            "source_artifact_sha256": self.source_artifact_sha256,
            "source_digest_scope": (
                "selected payload hashes, selected source bounds, MotionCrafter revision, "
                "lineage declaration, prediction-affecting settings, and metric anchor"
            ),
        }

    def run_summary(self, *, causal_frame_stop: int) -> dict[str, Any]:
        """Return an operational report that may mention unconsumed future entries."""

        return {
            "prediction_manifest": str(self.manifest_path),
            "prediction_manifest_sha256": self.manifest_sha256,
            "causal_frame_stop_exclusive": causal_frame_stop,
            "selected_window_ids": [item.window_id for item in self.windows],
            "selected_window_count": len(self.windows),
            "skipped_future_or_crossing_window_count": self.skipped_window_count,
            "future_prediction_payloads_opened": 0,
...
def _validate_overlap_lineage(manifest: Mapping[str, Any]) -> None:
    ...
    if not isinstance(lineage, Mapping):
        raise ValueError(
            "causal observation export requires an explicit temporal_lineage declaration"
        )
    if int(lineage.get("schema_version", -1)) != MOTIONCRAFTER_LINEAGE_SCHEMA_VERSION:
    ...
    if overlap.get("window_size_source") != "prediction archive frame count":
        raise ValueError("overlap-window source extent is not explicitly declared")
...
def _causal_source_digest(
    ...
    *,
    ...
    descriptor = {
        ...
        "selected_windows": [window.lineage_record() for window in windows],
...
def select_causal_overlap_windows(
    ...
    *,
    ...
    """Open only independently decoded windows wholly before the causal cutoff."""
    ...
    if frame_stride < 1 or window_size < 1:
        raise ValueError("prediction manifest has invalid frame_stride or window_size")
    ...
    for manifest_index, entry in enumerate(entries):
        if not isinstance(entry, Mapping):
            raise ValueError("overlap-window manifest entries must be mappings")
        window_id = str(entry.get("window_id", ""))
        if not window_id or window_id in seen_ids:
            raise ValueError("overlap-window IDs must be nonempty and unique")
        seen_ids.add(window_id)
        start = int(entry.get("start_frame", -1))
        stop = int(entry.get("stop_frame", -1))
        if start < 0 or stop <= start:
            raise ValueError(f"overlap window {window_id!r} has invalid source bounds")
    ...
    if not selected:
        raise ValueError(
            "no independently decoded overlap window is wholly before causal_frame_stop"
        )
    if selected[0].window_id != metric_anchor.window_id:
        raise ValueError(
