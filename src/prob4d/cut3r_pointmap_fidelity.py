...
def _validated_members(root: Path) -> dict[str, dict[int, Path]]:
...
def _geometry_fidelity(
    ...
    *,
    ...
    if point_count == 0:
        raise ValueError("CUT3R fidelity audit has no common supported point")
    medians = np.asarray(
        [item["median_error_m"] for item in per_frame if item["median_error_m"] is not None],
        dtype=np.float64,
    )
...
def build_cut3r_pointmap_fidelity_report(
    ...
    *,
    ...
    equivalent = (
        fidelity["point_weighted_rms_error_m"] <= maximum_rms
        and fidelity["maximum_frame_p95_error_m"] <= maximum_p95
    )
...
def load_cut3r_pointmap_fidelity_report(path: str | Path) -> dict[str, Any]:
...
def verify_cut3r_pointmap_fidelity_report(
    ...
    replay = build_cut3r_pointmap_fidelity_report(
        prefix_root,
        extended_root,
        prefix_frame_count=_positive_integer(
            report["prefix_frame_count"],
        ...
        maximum_rms_error_m=_finite_nonnegative(
            policy.get("maximum_rms_error_m"),
            name="maximum_rms_error_m",
        ),
        maximum_frame_p95_error_m=_finite_nonnegative(
            policy.get("maximum_frame_p95_error_m"),
            name="maximum_frame_p95_error_m",
        ),
        point_closure_tolerance_m=_finite_nonnegative(
            closure.get("point_tolerance_m"),
            name="point_tolerance_m",
        ),
        scalar_closure_tolerance=_finite_nonnegative(
            closure.get("scalar_tolerance"),
            name="scalar_tolerance",
        ),
        camera_closure_tolerance=_finite_nonnegative(
            closure.get("camera_tolerance"),
            name="camera_tolerance",
        ),
    )
    if replay != report:
