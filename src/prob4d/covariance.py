...
def validated_covariance_psd(
    ...
    *,
    ...
    """Return a defensive validated PSD covariance without adding a floor.

    Materially asymmetric or indefinite input fails closed. Negative eigenvalues
    that are within the declared floating-point tolerance are projected to zero.
    Exactly singular or zero covariance is otherwise preserved, so this helper is
    suitable for immutable model and artifact contracts rather than only for
    inverse operations.
    """
...
def covariance_eigendecomposition(
    ...
    *,
    ...
    return symmetric, np.maximum(eigenvalues, eigenvalue_floor), eigenvectors
