...
PUBLIC_API_MANIFEST_VERSION: Final = 2
PACKAGE_ROOT_SURFACE_VERSION: Final = 2
PACKAGE_ROOT_LOADING_MODE: Final = "minimal-version-root-v1"
...
def build_public_api_manifest() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "surfaces": {
            "package_root": {
                "module": "prob4d",
                "surface_version": PACKAGE_ROOT_SURFACE_VERSION,
                "loading": PACKAGE_ROOT_LOADING_MODE,
...
def validate_public_api_manifest(value: Any) -> dict[str, Any]:
    ...
    _exact_keys(surfaces, {"package_root", "api_v2"}, name="surfaces")

    root = _strict_mapping(surfaces["package_root"], name="package_root")
    _exact_keys(
        root,
        {"module", "surface_version", "loading", "exports"},
        name="package_root",
    )
    if _strict_string(root["module"], name="package_root.module") != "prob4d":
        raise ValueError("package-root module is not canonical")
    if (
        _strict_integer(root["surface_version"], name="package_root.surface_version")
        != PACKAGE_ROOT_SURFACE_VERSION
    ):
        raise ValueError("package-root surface version is not supported")
    if (
        _strict_string(root["loading"], name="package_root.loading")
        != PACKAGE_ROOT_LOADING_MODE
    ):
        raise ValueError("package-root loading mode is not canonical")
    if _validate_exports(root["exports"], name="package_root.exports") != [
        "__version__"
    ]:
        raise ValueError("package root must export only __version__")
    ...
    if _strict_string(api2["lifecycle"], name="api_v2.lifecycle") != "current":
        raise ValueError("api_v2 lifecycle is not canonical")
...
def _parser() -> argparse.ArgumentParser:
    ...
    show = subparsers.add_parser(
        "print",
        help="print the executing installation's manifest",
    )
    show.set_defaults(handler=_print_command)

    build = subparsers.add_parser(
        "build",
        help="write the executing installation's manifest",
    )
