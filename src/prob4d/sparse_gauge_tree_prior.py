...
class SparseGaugeTreePrior:
    ...
    def from_dense_covariance(
        ...
        *,
        ...
        if not np.allclose(
            ...
            maximum_error = float(
                np.max(np.abs(reconstructed - matrix), initial=0.0)
            )
            raise ValueError(
    ...
    def from_joint_gauge_posterior(
        ...
        *,
        ...
        positions = {
            window_id: index for index, window_id in enumerate(window_ids)
        }
    ...
    def apply_square_root_transpose(self, value: Any) -> FloatArray:
        ...
        for child in range(self.gauge_count - 1, -1, -1):
            ...
            if parent >= 0:
                accumulated[parent] += (
                    self.transition_matrices[child].T @ accumulated[child]
                )
        return _restore_gauge_rhs(result, layout)
    ...
    def apply_information(self, value: Any) -> FloatArray:
        ...
        for child in range(self.gauge_count):
            ...
            if parent >= 0:
                gradient[parent] -= (
                    self.transition_matrices[child].T @ weighted
                )
    ...
    def _resolve_gauges(
        self,
        gauges: Sequence[int | str],
        *,
        name: str,
    ) -> tuple[int, ...]:
        positions = {
            window_id: index for index, window_id in enumerate(self.window_ids)
        }
        resolved: list[int] = []
        for value in gauges:
    ...
    def apply_observation_covariance(
        ...
        if jacobians.ndim != 3 or jacobians.shape[1:] != (
            3,
            GAUGE_DIMENSION,
        ):
        ...
        if indices.shape != (len(jacobians),) or indices.dtype.kind not in {
            "i",
            "u",
        }:
