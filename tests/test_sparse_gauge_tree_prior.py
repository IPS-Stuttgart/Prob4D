...
def _dense_observation_design(
    ...
    design = np.zeros((3 * len(jacobians), 7 * gauge_count), dtype=np.float64)
...
def test_dense_square_root_and_covariance_are_exact() -> None:
    ...
    np.testing.assert_allclose(covariance, root @ root.T, atol=1e-11, rtol=1e-11)
...
def test_joint_posterior_adapter_requires_and_preserves_tree_lineage() -> None:
    ...
    np.testing.assert_allclose(admitted.dense_covariance(), prior.dense_covariance())
...
def test_information_actions_and_log_determinant_match_dense_algebra() -> None:
    ...
    assert sign > 0.0
    assert prior.log_determinant() == pytest.approx(dense_log_determinant, abs=2e-10)
...
def test_sparse_observation_covariance_action_matches_dense_design() -> None:
    ...
    gauge_indices = generator.integers(0, prior.gauge_count, size=observation_count)
    value = generator.normal(size=(3 * observation_count, 3))
    design = _dense_observation_design(jacobians, gauge_indices, prior.gauge_count)
    expected_covariance = design @ prior.dense_covariance() @ design.T

    np.testing.assert_allclose(
        prior.apply_observation_covariance(jacobians, gauge_indices, value),
...
def test_dense_admission_rejects_correlated_sibling_innovations() -> None:
    prior = _random_prior(8, gauge_count=5)
    root = prior.dense_square_root()
...
def test_storage_is_linear_and_randomized_tree_parity_is_stable() -> None:
    ...
    for seed in range(20):
        ...
        right_hand_side = generator.normal(size=(candidate.dense_dimension, 2))
        np.testing.assert_allclose(
