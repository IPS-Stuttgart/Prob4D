...
import prob4d.prediction_provider_io as provider_io
from prob4d.data import PredictionWindow
from prob4d.prediction_provider_io import (
...
def _write_window(path: Path, *, window_id: str = "window-0") -> None:
...
def _manifest(payload: Path, *, sequence_id: str = "case-a") -> PredictionProviderManifestV1:
...
def test_serialized_idempotent_manifest_persistence(tmp_path: Path) -> None:
    payload = tmp_path / "window.npz"
    _write_window(payload)
    manifest = _manifest(payload)
    destination = tmp_path / "provider.json"

    save_prediction_provider_manifest(destination, manifest)
    original = destination.read_bytes()
    save_prediction_provider_manifest(destination, manifest)

    assert destination.read_bytes() == original
    assert load_prediction_provider_manifest(destination).artifact_id == manifest.artifact_id
...
def test_existing_different_manifest_is_not_replaced(tmp_path: Path) -> None:
    payload = tmp_path / "window.npz"
    _write_window(payload)
    destination = tmp_path / "provider.json"
    save_prediction_provider_manifest(destination, _manifest(payload))
    ...
    with pytest.raises(ValueError, match="refusing to replace"):
        save_prediction_provider_manifest(
            destination,
            _manifest(payload, sequence_id="case-b"),
        )
    assert destination.read_bytes() == original
...
def test_payload_mutation_during_verification_is_rejected(
    ...
    original = provider_io.PredictionWindow.from_npz
...
def test_manifest_mutation_during_verification_is_rejected(
    ...
    manifest_path = tmp_path / "provider.json"
    save_prediction_provider_manifest(manifest_path, _manifest(payload))
    original = provider_io._load_prediction_provider_manifest
    ...
    monkeypatch.setattr(
        provider_io,
        "_load_prediction_provider_manifest",
        mutating_loader,
    )
