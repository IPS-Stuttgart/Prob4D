...
def _window(
    ...
    *,
    ...
    window = PredictionWindow(
        ...
        point_map=point_map,
        valid_mask=valid,
        scene_flow=np.zeros_like(point_map) if scene_flow else None,
        deform_mask=valid,
...
def _missing_future_descriptor() -> PredictionPayloadDescriptorV1:
    return PredictionPayloadDescriptorV1(
        product_role="independent-window",
        window_id="future",
...
def test_compatible_batch_passes_and_roundtrips(tmp_path: Path) -> None:
    ...
    first = _window(first_path, window_id="first")
    second = _window(second_path, window_id="second")
    ...
    write_prediction_batch_preflight(output, result)
    loaded = load_prediction_batch_preflight(output)
...
def test_spatial_mismatch_is_a_structured_negative(tmp_path: Path) -> None:
    first_path = tmp_path / "first.npz"
    second_path = tmp_path / "second.npz"
    first = _window(first_path, window_id="first", height=2)
    second = _window(second_path, window_id="second", height=4)
    manifest_path = _manifest(
        tmp_path / "provider.json",
        (
            _descriptor(
    ...
    assert violation.expected == [2, 3]
    assert violation.observed == [4, 3]


def test_policy_can_explicitly_admit_ragged_spatial_support(tmp_path: Path) -> None:
    first_path = tmp_path / "first.npz"
    second_path = tmp_path / "second.npz"
    first = _window(first_path, window_id="first", height=2)
    second = _window(second_path, window_id="second", height=4)
...
def test_future_payload_bytes_are_not_opened(tmp_path: Path) -> None:
...
def test_selected_payload_integrity_failure_is_not_a_scientific_negative(
    tmp_path: Path,
) -> None:
