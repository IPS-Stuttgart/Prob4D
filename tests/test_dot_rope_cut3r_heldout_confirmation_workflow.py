...
def test_confirmation_workflow_is_single_request_triggered_and_stage_sealed() -> None:
    ...
    assert "old confirmation provider attempt {attempt} may have started;" in text
    assert "for attempt in range(1, attempt_count + 1):" in text
    assert (
        'f"/actions/runs/{run_id}/attempts/{attempt}/jobs?per_page=100"'
        in text
    )
    assert 'run["run_attempt"] != 2' not in text
    assert "old confirmation produced an artifact; recovery is forbidden" in text
...
def test_confirmation_provider_reuses_audited_curope_compatibility_patch() -> None:
    ...
    assert 'git -C "$checkout" apply "$patch_path"' in provider
    assert "tokens.scalar_type()" in provider
    assert "tokens.type()" in provider
    assert "arch=compute_89,code=sm_89" in provider
    assert "curope-build-compatibility.json" in provider
    assert '"dot_payload_opened": False' in provider
    assert '"provider_prediction_run": False' in provider
    assert '"marker_payload_opened": False' in provider
    assert '"scientific_inputs_changed": False' in provider
