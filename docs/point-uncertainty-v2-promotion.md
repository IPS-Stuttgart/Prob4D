...
2. fit and seal the candidate `PointUncertaintyCalibrationV2` on its complete
   source/calibration group roster;
3. freeze the matched production-v1 point calibration using the **same independent
   training groups**;
4. freeze the disjoint validation object/session roster and prediction-only
   feature contract; and
5. freeze a promotion policy with all proper-score, calibration, width, and
...
frozen prediction-only feature contract. Shared `Sim(3)` gauge covariance remains
outside both local point-covariance models.
