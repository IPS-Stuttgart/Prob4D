...
def _load_protocol(path: Path) -> dict[str, Any]:
    ...
    if any(second <= first for first, second in zip(strengths, strengths[1:], strict=False)):
...
def select_strength(
    ...
    for strength in strengths:
        ...
        by_sequence = {
            str(row["sequence"]): float(row["normalized_nll_per_dimension"]) for row in rows
        }
...
def calibrate(args: argparse.Namespace) -> int:
    ...
    result: dict[str, Any] = {
        ...
        "confirmation_reserved_after_candidate": protocol["confirmation_reserved_after_candidate"],
